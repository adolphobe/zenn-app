...
import { ArrowRight, ArrowUpRight, CheckCircle2 } from 'lucide-react'; // X removido daqui, será usado no ExplanationModal
import { Button } from '@/components/ui/button';
import { Card, CardContent } from '@/components/ui/card';
import LoginForm from '@/components/LoginForm';
import ExplanationModal from '@/components/ExplanationModal'; // Importar o novo modal
...
const modalDataContent = {
    pilares: {
      ...
      content: (
        // Conteúdo JSX para 'pilares' como definido anteriormente
        // Idealmente, os parágrafos e títulos aqui teriam classes Tailwind para consistência
        // Ex: <h4 className="text-lg font-semibold text-blue-600 dark:text-blue-400 mb-1">Importância real (Risco de não fazer)</h4>
        // <p className="mb-3 text-gray-600 dark:text-gray-300">...</p>
        <div className="space-y-3">
          <p className="font-medium">No Zenn, você não adiciona uma tarefa apenas com um título e uma data. Você adiciona consciência.</p>
          <p>Cada tarefa é avaliada com base em três pilares:</p>
          <div className="pl-4 space-y-2 border-l-2 border-blue-100 dark:border-gray-700">
            <div>
              <h4 className="font-semibold text-blue-700 dark:text-blue-400">Importância real (Risco de não fazer)</h4>
              <p className="italic text-sm text-gray-500 dark:text-gray-400">“Se eu ignorar isso, vai ter consequência?”</p>
              <p>É o que te faz priorizar o que realmente não pode ser adiado.</p>
            </div>
            <div>
              <h4 className="font-semibold text-blue-700 dark:text-blue-400">Orgulho pós-execução</h4>
              <p className="italic text-sm text-gray-500 dark:text-gray-400">“Quando eu terminar, vou me sentir mais alinhado com quem quero ser?”</p>
              <p>É o pilar que mede sua conexão com o que você faz. Não é só cumprir — é se respeitar por ter feito.</p>
            </div>
            <div>
              <h4 className="font-semibold text-blue-700 dark:text-blue-400">Crescimento pessoal</h4>
              <p className="italic text-sm text-gray-500 dark:text-gray-400">“Fazer isso me torna alguém melhor?”</p>
              <p>É o que separa tarefas operacionais de tarefas que constroem a sua evolução real.</p>
            </div>
          </div>
          <p className="font-medium pt-2">Esses três pilares não são para julgar produtividade. Eles te ajudam a escolher com consciência o que realmente merece seu tempo.</p>
        </div>
      )
    },
    clareza: {
      ...
      content: (
        <div className="space-y-3">
          <p className="font-medium">Todo mundo tem uma lista de tarefas. O Zenn não é mais uma. Ele é o primeiro sistema que organiza a intenção por trás de cada escolha.</p>
          ...
          <ul className="list-disc list-inside space-y-1 text-blue-700 dark:text-blue-400 font-medium pl-2">
          ...
          <p className="font-medium pt-2">Resultado? Menos culpa. Mais direção.</p>
        </div>
      )
    },
    estrategia: {
      ...
      content: (
        <div className="space-y-3">
          <p className="font-medium">Você não precisa esperar uma crise para perceber que está vivendo no automático. O Zenn analisa, em tempo real, o que suas tarefas estão dizendo sobre você.</p>
          ...
          <p className="font-medium pt-2">Não é só um app de tarefas. É um sistema de leitura da sua própria execução.</p>
        </div>
      )
    }
  };
...
const Landing: React.FC = () => {
  ...
  const openExplanationModal = (type: ModalContentType) => {
    ...
    // document.body.style.overflow = 'hidden'; // O Dialog do Radix geralmente cuida disso
  };

  const closeExplanationModal = () => {
    setIsExplanationModalOpen(false);
    setCurrentModalData(null);
    // document.body.style.overflow = 'auto'; // O Dialog do Radix geralmente cuida disso
  };

  // Efeito para fechar modal com ESC (O Dialog do Radix já faz isso, mas pode manter se quiser redundância ou para modais customizados)
  // useEffect(() => {
  ...
  //     }
  //   };
  //   if (isExplanationModalOpen) { // Adiciona listener apenas se o modal estiver aberto
  ...
  useEffect(() => {
    ...
    return () => document.querySelectorAll('.animate-on-scroll').forEach(section => observer.unobserve(section));
  }, []);
  ...
  return (
    <div className="min-h-screen overflow-x-hidden">
      <style>
        {`
          /* ... Seu CSS existente ... */
          /* Adicionar estilos para .prose se necessário para dark mode, ou confiar no dark:prose-invert */
        `}
      </style>
      
      {/* Hero Section e outras seções... */}
      {/* ... */}
     ...
     <section className="py-32 relative z-10 overflow-hidden bg-gradient-to-b from-white via-blue-100 to-white dark:from-gray-900 dark:via-gray-800 dark:to-gray-900" style={{backfaceVisibility: 'hidden', transform: 'translateZ(0)'}}>
      ...
      {/* ... Footer ... */}
      {/* COLE O RESTANTE DO ARQUIVO A PARTIR DAQUI (Seção How It Works até o Footer) */}
