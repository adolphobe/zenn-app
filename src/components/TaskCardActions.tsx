    ...
    <div className="flex justify-between space-y-0 space-x-2 mt-4">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {viewMode === 'power' && (
          ...
          >
            {isHidden ? 
              <><Eye size={16} className="text-green-600" />{!isMobile && <span className="ml-1">Mostrar</span>}</> : 
              <><EyeOff size={16} />{!isMobile && <span className="ml-1">Ocultar</span>}</>
            }
          </Button>
        )}
        ...
        {viewMode === 'power' && showPowerExtraButton && (
          ...
          >
            ...
            {!isMobile && <span className="ml-1">{isPowerExtra ? "Desativar PE" : "Potência Extra"}</span>}
          </Button>
        )}
      </div>
      
      {/* Right side action: Complete - Always visible */}
      <Button
        variant="outline"
        size="sm"
